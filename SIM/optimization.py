...
class SparseOptimizer:
    def __init__(self, cfg, X, U, _params, verbose):
        self.cfg = cfg
        ...
        obj = cp.Minimize(cfg.alpha * cp.sum(self.s) + cfg.lambda1*cp.norm(params[0].detach().numpy()-XU@cp.hstack([self.a, self.b]))**2)
        # easy constraint!
...
def build_optimizer(cfg, X, U, params, verbose):
    verbose = True if verbose == 0 else False
    if cfg.objective == "sparsity":
        return SparseOptimizer(cfg, X, U, params, verbose)
    if cfg.objective == "robustness":
        return RobustOptimizer(cfg, X, U, params, verbose)
